...
if($cfg_curmode == 'small')
{
	if(!isset($cid))
	{
		...
		//parentid一般是通过修改页面得到
		else if(isset($parentid)) $cid = $parentid;
		
		else $cid = 0;

		//获取顶部id
		$r = $dosql->GetOne("SELECT `parentstr` FROM `$tbname` WHERE id=$cid");
		$r2 = explode(',',$r['parentstr']);
		if($r2[1] != '') $cid = $r2[1];
	}

	$gourl = 'infoclass_user.php?cid='.$cid;
}
else
{
	$gourl = 'infoclass.php';
...
if($action == 'add')
{
	...
	$sql = "INSERT INTO `$tbname` (siteid, parentid, parentstr, infotype, classname, linkurl, picurl, picwidth, picheight, seotitle, keywords, description, orderid, checkinfo, adminmenu, menulevel) VALUES ('$cfg_siteid', '$parentid', '$parentstr', '$infotype', '$classname', '$linkurl', '$picurl', '$picwidth', '$picheight', '$seotitle', '$keywords', '$description', '$orderid', '$checkinfo', '$adminmenu', '$menulevel')";
...
else if($action == 'update')
{
	...
	if($cfg_curmode != 'small')
	{
		$sql = "UPDATE `$tbname` SET siteid='$cfg_siteid', parentid='$parentid', parentstr='$parentstr', infotype='$infotype', classname='$classname', linkurl='$linkurl', picurl='$picurl', picwidth='$picwidth', picheight='$picheight', seotitle='$seotitle', keywords='$keywords', description='$description', orderid='$orderid', checkinfo='$checkinfo', adminmenu='$adminmenu', menulevel='$menulevel' WHERE id=$id";
	}
	else
	{
