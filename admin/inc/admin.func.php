...
function GetAllType($tbname='', $tbname2='', $colname='', $id=0, $i=0)
{
	...
	if($tbname != '#@__goodstype' &&
	   $tbname != '#@__goodsbrand')
		$sql = "SELECT * FROM `$tbname` WHERE `siteid`='$cfg_siteid' AND `checkinfo`=true AND `parentid`=$id ORDER BY orderid ASC";
	else
		$sql = "SELECT * FROM `$tbname` WHERE `checkinfo`=true AND `parentid`=$id ORDER BY orderid ASC";
...
function GetCatpSize($t=0)
{
	...
	if(!empty($_GET['id']))
	{
		$r = $dosql->GetOne("SELECT `picwidth`,`picheight` FROM `#@__infoclass` WHERE `id`=".$_GET['id']);
