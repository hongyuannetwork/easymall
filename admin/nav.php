...
<form name="form" id="form" method="post" action="">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
		<tr align="center" class="thead">
			...
			<td width="20%" class="title2">导航名称</td>
			<td width="15%">跳转链接</td>
			<td width="10%">ID</td>
		<td width="15%">排序</td>
			<td width="25%">操作</td>
	...
	<div rel="rowpid_<?php echo $topid ; ?>">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
			...
			<td width="25%" class="action"><span><a href="nav_add.php?id=<?php echo $row['id']; ?>">添加子导航</a></span> | <span><a href="nav_save.php?action=check&id=<?php echo $row['id']; ?>&checkinfo=<?php echo $row['checkinfo']; ?>" title="点击进行显示与隐藏操作"><?php echo $checkinfo; ?></a></span> | <span><a href="nav_update.php?id=<?php echo $row['id']; ?>">修改</a></span> | <span><a href="nav_save.php?action=del&id=<?php echo $row['id'] ?>" onclick="return ConfDel(2);">删除</a></span></td>
			</tr>
		</table>
	</div>
	<?php
