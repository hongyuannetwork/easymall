...
if($action == 'save')
{
	...
	$sql = "INSERT INTO `#@__goodsorder` (username, attrstr, truename, idcard, telephone, zipcode, postarea_prov, postarea_city, postarea_country, address, ordernum, postmode, paymode, getmode, weight, amount, buyremark, posttime, orderid, checkinfo) VALUES ('$username', '$attrstr', '$truename', '$idcard', '$telephone', '$zipcode', '$postarea_prov', '$postarea_city', '$postarea_country', '$address', '$ordernum', '$postmode', '$paymode', '$getmode', '$weight', '$amount', '$buyremark', '$posttime', '$orderid', 'confirm')";
	...
	if($dosql->ExecNoneQuery($sql))
	{
		...
		if($paymode == 1)
		{
			header('location:orderpay.php?id='.$dosql->GetLastID(),'ENCODE');
			exit();
		}
    	else
		{
